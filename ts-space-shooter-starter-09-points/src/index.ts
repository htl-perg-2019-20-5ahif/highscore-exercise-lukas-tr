...
const sendHighscores = async (points) => {
  try{
  const response = await fetch("/api/HighscoreEntries", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({name: (document.querySelector("#name") as any).value, points }) 
  });
...
const displayHighscores = async () => {
  const resp = await fetch("/api/HighscoreEntries");
  const scores = await resp.json();
  document.getElementById("scores").innerHTML = scores.map((s, i) => `${i+1}. ${s.name} - ${s.points}`).join("<br>")
};
...
class ShooterScene extends Scene {
  ...
  gameOver() {
    ...
    if (!gameOver) {
      gameOver = true;
      sendHighscores(this.hits).then(displayHighscores);
...
(window as any).startGame = () => {
  const name = (document.querySelector("#name") as any).value;
  if (name && name.length >= 3) {
    (document.querySelector("#hide-on-start") as any).style.display = "none";
    new Game(config);
  } else {
    alert("invalid name");
  }
